...
public class OfferService {
    ...
    public void createOffer(OfferRequestDto offerDto, String currentUserId) {
        ...
        // [✅✅✅ 핵심 수정 1/2] 요청이 OPEN 상태인지 반드시 확인합니다.
        if (request.getStatus() != RequestStatus.OPEN) {
            throw new IllegalStateException("이미 마감된 요청이므로 제안할 수 없습니다.");
        }
        
        // [✅✅✅ 핵심 수정 2/2] 마감 시간이 지났는지도 서버 시간 기준으로 다시 한번 확인합니다.
        if (LocalDateTime.now().isAfter(request.getDeadline())) {
            // 참고: 클라이언트 시간과 서버 시간 차이로 인해 발생할 수 있는 엣지 케이스 방지
            throw new IllegalStateException("마감 시간이 지난 요청이므로 제안할 수 없습니다.");
        }
